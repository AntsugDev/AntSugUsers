...
use App\Http\Api\Google\Resource\AccessGoogleResource;
...
class WhoAmIController extends Controller
{
    ...
    public function who_am_i(Request $request)
    {
        ...
        }else if($requestAuthenticatable instanceof AccessGoogle)
            return(new AccessGoogleResource($this->load($requestAuthenticatable)))->response(request())->setStatusCode(Response::HTTP_OK);
